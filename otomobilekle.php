...
<body>
    ...
    if ($_POST) {
        ...
        if (!in_array($image_ext, array("jpg", "jpeg", "png", "gif"))) {
            ...
            $ekle = mysqli_query($conn, "INSERT INTO otomobil (marka, model, renk, km, resim, fiyat) VALUES ('$marka', '$model', '$renk', '$km', '$image_path','$fiyat')");
            
            if($ekle) {
                echo "<script>alert('Otomobil başarıyla eklendi!');</script>";
              echo "<script>window.location.href = 'adminotomobil.php';</script>";
            }
        }
    }
?>